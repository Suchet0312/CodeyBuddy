...
class CodeLensState(TypedDict):
    ...
    # Investigation trace.
    # The add reducer appends new entries
...
def rerank_node(state):
    ...
    return {
        ...
        "investigation_history": [
            "Reranking completed. "
            f"Selected top {len(reranked_documents)} documents."
        ]
    }
...
a code repository investigation system.

The current retrieval query did not retrieve enough
evidence to answer the user's original question.

Your task is to create a better search query for
retrieving relevant source code.
...
Create one improved search query that:

- preserves the user's original intent
- uses important technical keywords
- includes likely function, class, module, or implementation terms
- is optimized for searching a code repository
...
def insufficient_evidence_node(state):
    ...
    answer = (
        ...
        "retrieved code to answer the question: "
        f"'{original_question}'. "
        "The available code does not appear to contain "
        "the required implementation."
    )

    print(
        "Returning insufficient evidence response."
    )
...
graph_builder.add_conditional_edges(
    ...
    decide_route,
    {
        "direct": "dense_retrieve",
        "investigate": "hybrid_retrieve"
    }
)
...
question = (
    "How is email verification implemented?"
)
